    ...
    <div class="container-fluid">
      ...
      <div class="row">
        ...
        <?php
                 ...
                 $sqlCombo = "SELECT `tipo`.`tipo_pk`, `tipo`.`nome` FROM `tipo` JOIN `curso` ON `curso`.`tipo_fk`=`tipo`.`tipo_pk` JOIN `nivel` ON  `nivel`.`nivel_pk`=`curso`.`nivel_fk` JOIN `curso_disciplina` ON `curso_disciplina`.`curso_fk_pk`=`curso`.`curso_pk` JOIN `disciplina` ON `disciplina`.`disciplina_pk`=`curso_disciplina`.`disciplina_fk_pk` JOIN `departamento` ON `departamento`.`departamento_pk`=`disciplina`.`departamento_fk` WHERE `nivel`.`nivel_pk` = '" . $_COOKIE['nivel'] . "' AND `departamento`.`departamento_pk` = '" . $_COOKIE['departamento'] . "' AND `tipo`.`ativo` = 1 GROUP BY `tipo`.`nome` ";
                 $resultCombo = $conn->query($sqlCombo);
                 if(isset($_POST['combotipo'])){
                     setcookie("tipo",$_POST['combotipo']);
                     header("Location: http://" .$_SERVER['HTTP_HOST']. "/m17e/criteriospdisciplina4.php");
                 }
                 if($resultCombo->num_rows<=0) {
                    echo("<script>alert('Não existem Tipos!');</script>");
                 }else{     
                    echo("<form method='POST'>"
                                . "<table>"
                                    . "<tr><td>");
                                        ...
                                        echo("<select id='combo' name='combotipo'>");
                                        for($i=0; $i < count($preencheCombo); $i++){
                                            echo("<option value='" . $preencheCombo[$i]["tipo_pk"] . "'> ". $preencheCombo[$i]["nome"] ." </option> ");
                                        }
                                        echo(" </select> ");
                                    echo("</td>");
