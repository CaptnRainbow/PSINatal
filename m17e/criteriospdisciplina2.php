    ...
    <div class="container-fluid">
      ...
      <div class="row">
        ...
        <?php
                 ...
                 $sqlCombo = "SELECT `nivel`.`nivel_pk`, `nivel`.`nome` FROM `nivel` JOIN `curso` ON `curso`.`nivel_fk`=`nivel`.`nivel_pk` JOIN `tipo` ON  `tipo`.`tipo_pk`=`curso`.`tipo_fk` JOIN `curso_disciplina` ON `curso_disciplina`.`curso_fk_pk`=`curso`.`curso_pk` JOIN `disciplina` ON `disciplina`.`disciplina_pk`=`curso_disciplina`.`disciplina_fk_pk` JOIN `departamento` ON `departamento`.`departamento_pk`=`disciplina`.`departamento_fk` WHERE `departamento`.`departamento_pk` = '" . $_COOKIE['departamento'] . "' AND `nivel`.`ativo` = 1 GROUP BY `nivel`.`nome`";
                 $resultCombo = $conn->query($sqlCombo);
                 if(isset($_POST['combonivel'])){
                     setcookie("nivel",$_POST['combonivel']);
                     header("Location: http://" .$_SERVER['HTTP_HOST']. "/m17e/criteriospdisciplina3.php");
                 }
                 if($resultCombo->num_rows<=0) {
                    echo("<script>alert('Não existem Niveis!');</script>");
                 }else{     
                    echo("<form method='POST'>"
                                . "<table>"
                                    . "<tr><td>");
                                        ...
                                        echo("<select id='combo' name='combonivel'>");
